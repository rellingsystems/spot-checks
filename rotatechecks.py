#!/usr/bin/env python3
"""
Generate spot check overlays for _rot_720p videos only.
- Reads from perfect_frame_annotation_rot.json
- Extracts frames from _rot_720p videos
...
def add_legend(frame, video_name, frame_num):
    ...
    # Frame info
    cv2.putText(frame, f"Frame: {frame_num} | Source: _rot_720p",
...
def process_video(category, video_name, json_data, output_dir):
    ...
    # Build _rot_720p video path
    video_file = f"{video_name}_rot_720p.mp4"
    ...
    # Save
    output_path = os.path.join(output_dir, f"{video_name}_rot_spot_check.png")
...
def main():
    parser = argparse.ArgumentParser(description='Generate spot checks for _rot_720p videos')
    parser.add_argument('--json', type=str, default='perfect_frame_annotation_rot.json',
    ...
    print("=" * 60)
    print("SPOT CHECK GENERATOR - _rot_720p Videos Only")
    print("=" * 60)
